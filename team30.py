...
class Team30():
	...
	def heuristic(self, board, flag):
		...
		for i in range(4):
			for j in range(4):
				block = tuple(self.find_block(board.board_status, 4*i, 4*j))
				if block in self.hashtable:
					# print('hit')
					board_prob[i][j] = self.hashtable[block]
				else:
					self.hashtable[tuple(self.find_block(board.board_status, 4*i, 4*j))] = self.find_prob_cells(board.board_status,4*i,4*j)
	...
	def find_block(self, board_status, topx, topy):
		arr = []
		for i in range(topx, topx+4):
			for j in range(topy, topy+4):
				arr.append(board_status[i][j])
		return arr
